...
\begin{document}
	...
	\begin{frame}{Scaling to Higher Load}
		\begin{itemize}
			\pitem \textbf{Scaling Approaches}
			\begin{itemize}
				\pitem Vertical Scaling
				\pitem Horizontal Scaling
			\end{itemize}

			\pitem \textbf{Scaling Architectures}
			\begin{itemize}
				\pitem Shared-Memory
				\pitem Shared-Disk
				\pitem Shared-Nothing
			\end{itemize}
		\end{itemize}
	\end{frame}

	\begin{frame}{Reasons To Distribute Data}
		\begin{itemize}
			\pitem Scalability
			\pitem Reliability
			\pitem Performance
	...
	\begin{frame}[plain,c]
		\begin{center}
		\Huge What is Data Replication, Leaders and Followers?
		\end{center}
	\end{frame}
	...
	\begin{frame}{Single-Leader Replication}
		...
		\begin{itemize}
			\pitem \textbf{What happens when the leader fails?}
			\pitem \textbf{Databases with single-leader replication support}
			\begin{itemize}
	...
	\begin{frame}{Higher Availability}
		\begin{itemize}
			\pitem Sloppy Quorum
			\pitem Hinted Handoff
			\pitem $w + r > n$ can be inconsistent
			\pitem Availability \& Consistency Trade-Off
		\end{itemize}
	\end{frame}
...
for range 100000 {
  ...
  session.Query(q, uuid, "username", "mock@email.com").Exec()
	...
	\begin{frame}{References}
		...
		\tiny
		\begin{itemize}
			\item Designing Data-Intensive Applications Book (\url{https://tinyurl.com/3jer2jjn})
			\item Amdahl's Law Wiki Page (\url{https://tinyurl.com/5y4x4w3t})
			\item ScyllaDB Official Documentation (\url{https://www.scylladb.com/})
			\item ScyllaDB Setup/Source Example (\url{https://tinyurl.com/3z7dn2tc})
		\end{itemize}
	\end{frame}
\end{document}
